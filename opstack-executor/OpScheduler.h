...
public:
        ...
        std::function<void(bcos::Error::Ptr, protocol::TransactionReceipt::Ptr)> callback) override
    {
        task::wait([this, tx = std::move(transaction),
            ...
            {
                // RPC hygiene (round-2 F4): internal detail (storage schema, exception types,
                // throw locations) goes to the node log; the RPC-bound message stays generic.
            ...
            catch (...)
            {
                ...
                // Same generic RPC-bound wording as the typed catch (round-2 F4).
                cb(BCOS_ERROR_PTR(bcos::scheduler::SchedulerError::UnknownError,
                       "eth_call failed: internal error (see node log)"),
                    nullptr);
            }
        ...
        std::function<void(bcos::Error::Ptr, protocol::TransactionReceipt::Ptr)> callback) override
    {
        task::wait([this, tx = std::move(transaction), blockNumber,
            ...
            {
                // Same RTTI-bypass backstop as call(): -fno-rtti evmone types escape the typed
                // catch; normalize through describeException/classifyException. The RPC-bound
                // message is the SAME rpcSafeReason wording as the typed catch — which arm fires
                // is unreliable, so the observable text must not depend on it.
                auto const code = classifyException(std::current_exception());
                OP_SCHEDULER_LOG(WARNING)
                    << LOG_DESC("eth_call at block failed") << LOG_KV("block", blockNumber)
...
public:
    ...
    /// unreliable across the -fno-rtti evmone boundary (observed: an OpStorageError thrown after
    /// the executor co_await escapes catch(std::exception&) yet classifyException still types
    /// it), so the classification carries the semantics and this carries the wording.
...
private:
    ...
    /// @p errTag is only the message-prefix discriminator ("call" vs "historical").
        ...
        bcos::ledger::LedgerConfig const& ledgerConfig, std::string_view errTag)
    {
        ...
        if (!sharedError->empty())
            throw bcos::evm::engine::OpStorageError(
                fmt::format("OpScheduler: {} state read fault: {}", errTag, *sharedError));
