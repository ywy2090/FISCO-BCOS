...
| Old | New |
| --- | --- |
| `eth/execution/InnerExecute.h/.cpp` | `eth/execution/InnerExecute.h/.cpp` |
...
**Interfaces:**
- Consumes: nothing
- Produces: new include paths e.g. `#include "bcos-evm/eth/execution/InnerExecute.h"`

- [ ] **Step 1: Two-step git mv for case-only renames (macOS-safe)**

```bash
cd bcos-evm
git mv eth/execution/InnerExecute.h eth/_ExecuteMessage.h && git mv eth/_ExecuteMessage.h eth/execution/InnerExecute.h
...
In `eth/execution/InnerExecute.h`, change `@file ExecuteMessage.h` → `@file ExecuteMessage.h`.
