...
- **日期**：2026-08-20
- **状态**：已实现（③ + ①b + ①a 全部落地，2026-08-20；review 修正轮已并入）。`OpScheduler::callAtBlock`/`coCallAtBlock` + `MultiVersionScheduler` 透传；①b 全量 `stateRootOfWithNodes` 两层节点落盘（创世由 `Ledger::buildGenesisBlock` 负责，**execute() 的 block-0 全量回退已删除**——scenario-B 要求自创世开启 feature，父块必须已 commit）；①a 增量 `buildAndCollect` 已在 `OpScheduler::execute` 上线（`persistTrieNodes && number>0` 时走增量，root 取自增量构建，父根经 `getBlockData` 读父块头）。修正轮：coCallLatest/coCallAtBlock 补 poison/sharedError 响亮失败检查（吞错→`OpStorageError`）；`stateRootOfWithNodes` 加 `collectNodes` 开关（root-only 路径不驻留节点集）；`classifyException` 补 mpt 异常族（`MPTInvariantViolation`/`MPTDecodeError`）→ `OpStorageFault`（Storage2State poison 梯外的漏网读故障）。对拍门 `OpSchedulerTest.IncrementalMPTRootMatchesFullRebuild` + 端到端 `CallAtBlockServesEachHeightWithOpSemantics`（含块内 setter deposit 改 slot）+ 负向门（空 root / 无落盘节点→InvalidStatus，内部节点缺失→OpStorageFault）全绿；`opstack-executor-block-tests` 仅剩 2 个预存失败（缺 golden 文件 / 基线同挂，与 op-alignment 一致）。注意：①a 上线后**块外直写 flat state 会破坏六字段比对**（增量只认块 delta），测试态的 overwriteSlot 已改为块内 setter deposit
...
`OpScheduler::call()` → `coCallLatest`（`OpScheduler.h:1027`）：fork 最新存储 → 手建 `LedgerConfig` → `Storage2State` → `OpstackExecutor::executeTransaction(call=true)` → 丢弃 fork。
...
| M4 | `header` 必须绑定局部 `shared_ptr`（`auto blockHeader = block->blockHeader(); auto const& header = *blockHeader;`）否则悬垂 UB | 仿 `coCallLatest`（`OpScheduler.h:1040-1041` 防悬垂注释） |
...
EthereumExecutor::executeTransaction → applyToStorage 直写 flat（/apps/<40hex> + s_code_binary）
  ...
  → shouldBuildMPT ? → buildMPTStateRoot :351-371（读父块头 parentStateRoot）
...
| OP 语义保持 | 历史 receipt 含 OP 字段（l1_fee 等）——**deferred**：receipt OP 扩展字段为 FISCO 模拟产物、非 op-geth 对拍面（§8），当前以对拍门 + e2e 的 output/egp 断言覆盖 OP 语义主面，l1_fee 字段断言待补 |
| 历史 BLOCKHASH | 祖先哈希可回溯——**deferred**：`s_number_2_hash` 为 content-addressed 透传行（任意高度有效），历史 BLOCKHASH 读路径与 latest 相同；未设专门用例 |
...
- **scenario B 须自创世开启**（①a 增量构建前提）：在节点落盘存在之前已提交过 scenario-B 块的链，或中途激活 `feature_l2_ethereum_compat` 的链，会在激活/升级高度后的第一个块**停链**——⑤ 的增量构建发现父根无落盘节点时抛带命名诊断的 `OpStorageError`（指明父块号与根因），不会从空 trie 重建出分歧 root。恢复路径：从激活高度起重放，或补历史节点。
- **feature 激活高度在生产中恒为 0**：config 的 `feature_l2_ethereum_compat=1` 是**布尔**（`NodeConfig.cpp` `loadGenesisFeatures` 用 `get_value<bool>` 解析），创世由 `setGenesisFeatures` 以 activation=0 重写入 SYS_CONFIG（`Ledger.cpp:1813-1825`），非零激活在启动期被 `validateMPTFlagMatrix`（`LedgerInitializer.cpp:48`）拒绝。因此 block 0 的历史 eth_call 在真实 L2 链上正常服务（与 op-geth 一致，无分歧）；`CallAtBlockGenesisRefusedWhenFeatureActivatesAtOne` 以 enable=1 形态固定的是 feature 门"按 pin 高度读 flag"的纵深语义，不是生产可达路径。
- **历史读放大、无跨调用节点缓存**（round-2 F5，v1 接受）：每次历史 eth_call 都新建 `HistoricalStateBackend` 并从 "/mpt/" 行逐节点解析 trie，没有 op-geth 那样的跨调用 trie node cache 摊薄；同一高度反复调用的读成本与 trie 深度成正比。历史调用是低频 RPC 路径（对拍/调试/审计），v1 不引入缓存层；若未来出现热路径（如归档索引服务批量回放），再评估共享节点缓存。
