...
public:
    ...
    {
        BOOST_TEST(_prepareReq->block_hash == _decodedPrepareReq->block_hash);
        BOOST_TEST(_prepareReq->view == _decodedPrepareReq->view);
        BOOST_TEST(_prepareReq->height == _decodedPrepareReq->height);
        BOOST_TEST(_prepareReq->idx == _decodedPrepareReq->idx);
        ...
        BOOST_TEST(_decodedPrepareReq->pBlock->blockHeader() == _prepareReq->pBlock->blockHeader());
        BOOST_TEST(_decodedPrepareReq->pBlock->blockHeader().hash() ==
                   _prepareReq->pBlock->blockHeader().hash());
        BOOST_TEST(_decodedPrepareReq->pBlock->getTransactionSize() ==
...
BOOST_AUTO_TEST_CASE(testFetchMissedTxsAndFillBlock)
{
    ...
    *tmp = *commonBlock;
    ...
    BOOST_TEST(followsPrepareReq == followerReqCache->partiallyRawPrepare());
    ...
    BOOST_TEST(leaderReqCache->fetchMissedTxs(encodedTxsBytes, ref(*encodedMissTxsInfo)) == true);
    ...
    BOOST_TEST(followerReqCache->rawPrepareCache() == *followsPrepareReq);
    // check encode
    BOOST_TEST(followerReqCache->rawPrepareCache().block->size() > 0);
    ...
    {
        BOOST_TEST(*tx == *((*prepareReq->pBlock->transactions())[i]));
        BOOST_TEST(*tx == *((*tmpBlock->transactions())[i]));
    }
    ...
    BOOST_TEST(leaderReqCache->fetchMissedTxs(encodedTxsBytes, ref(*encodedMissTxsInfo)) == false);
