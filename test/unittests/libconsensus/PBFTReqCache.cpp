...
BOOST_AUTO_TEST_CASE(testAddAndExistCase)
{
    ...
    BOOST_TEST(req_cache.isExistPrepare(*prepare_req));
    ...
    BOOST_TEST(req_cache.isExistSign(*sign_req));
    auto fakeFutureReqCheckNum = 100;
    BOOST_TEST(req_cache.getSigCacheSize(sign_req->block_hash, fakeFutureReqCheckNum) == 1);
    /// test addCommitReq
    ...
    BOOST_TEST(req_cache.isExistCommit(*commit_req));
    BOOST_TEST(req_cache.getCommitCacheSize(commit_req->block_hash, fakeFutureReqCheckNum) == 1);
    /// test addPrepareReq
    ...
    BOOST_TEST(!req_cache.isExistSign(*sign_req));
    BOOST_TEST(!req_cache.isExistCommit(*commit_req));
    BOOST_TEST(req_cache.getSigCacheSize(sign_req->block_hash, fakeFutureReqCheckNum) == 0);
    BOOST_TEST(req_cache.getCommitCacheSize(commit_req->block_hash, fakeFutureReqCheckNum) == 0);

    /// test addFuturePrepareCache
    req_cache.addFuturePrepareCache(prepare_req);
    BOOST_TEST(req_cache.futurePrepareCacheSize() == 1);
    ...
    BOOST_TEST(req_cache.futurePrepareCacheSize() == 0);
    ...
    BOOST_TEST(req_cache.futurePrepareCacheSize() == 10);
    BlockHeader highest;
    highest.setNumber(8);
...
BOOST_AUTO_TEST_CASE(testSigListSetting)
{
    ...
    BOOST_TEST(req_cache.getCommitCacheSize(prepare_req->block_hash, node_num) == node_num);
    ...
    BOOST_TEST(ret);
    BOOST_TEST(block.sigList()->size() == node_num);
...
BOOST_AUTO_TEST_CASE(testCollectGarbage)
{
    ...
    BOOST_TEST(req_cache.getSigCacheSize(req->block_hash, validNum) == validNum);
    BOOST_TEST(req_cache.getSigCacheSize(invalid_hash, validNum) == 0);

    BOOST_TEST(req_cache.getCommitCacheSize(req->block_hash, validNum) == validNum);
    BOOST_TEST(req_cache.getCommitCacheSize(invalid_hash, validNum) == 0);
    /// test delCache
    req_cache.delCache(highest);
    BOOST_TEST(req_cache.getSigCacheSize(req->block_hash, validNum) == 0);
    BOOST_TEST(req_cache.getCommitCacheSize(req->block_hash, validNum) == 0);
}
...
BOOST_AUTO_TEST_CASE(testCanTriggerViewChange)
{
    ...
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req));
    req_cache.addViewChangeReq(viewChange_req2);
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req2));
    req_cache.addViewChangeReq(viewChange_req3);
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req3));
    VIEWTYPE minView;
    BOOST_TEST(req_cache.canTriggerViewChange(
        minView, maxInvalidNodeNum, toView, header, consensusBlockNumber));
    BOOST_TEST(minView == viewChange_req2->view);
    maxInvalidNodeNum = 3;
    BOOST_TEST(req_cache.canTriggerViewChange(
...
BOOST_AUTO_TEST_CASE(testViewChangeReqRelated)
{
    ...
    req_cache.addViewChangeReq(viewChange_req);
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req));
    BOOST_TEST(req_cache.getViewChangeSize(1) == 1);
    ...
    req_cache.addViewChangeReq(viewChange_req2);
    BOOST_TEST(req_cache.getViewChangeSize(1) == 2);
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req2));
    ...
    req_cache.addViewChangeReq(viewChange_req3);
    BOOST_TEST(req_cache.getViewChangeSize(1) == 3);
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req3));
    ...
    BOOST_TEST(req_cache.getViewChangeSize(1) == 0);
    ...
    BOOST_TEST(req_cache.isExistViewChange(*viewChange_req3));
    BOOST_TEST(req_cache.mutableSignCache().size() == 0);
    BOOST_TEST(req_cache.mutableCommitCache().size() == 0);
