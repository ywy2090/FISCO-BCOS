    ...
    SignReq& signReq, bool succ)
{
    if (!succ)
    {
        ...
        FakeValidNodeNum(fake_pbft, 4);
        BOOST_TEST(fake_pbft.consensus()->isValidSignReq(copiedReq) == CheckResult::FUTURE);
        BOOST_TEST(fake_pbft.consensus()->reqCache()->isExistSign(copiedReq) == true);
        BOOST_TEST(
            fake_pbft.consensus()->reqCache()->getSigCacheSize(copiedReq.block_hash, 1) == 0);
        BOOST_TEST(fake_pbft.consensus()->reqCache()->isExistSign(copiedReq) == false);
        ...
        BOOST_TEST(fake_pbft.consensus()->isValidSignReq(signReq) == CheckResult::INVALID);
        signReq.idx = org_idx;
        /// test invalid view
    ...
    CommitReq::Ptr commitReq, bool succ)
{
    if (!succ)
    {
        ...
        BOOST_TEST(fake_pbft.consensus()->isValidCommitReq(*commitReq) == CheckResult::INVALID);
        fake_pbft.consensus()->reqCache()->clearAll();
    ...
    SignReq::Ptr signReq, PrepareReq::Ptr prepareReq, KeyPair const& peer_keyPair, bool succ)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->isValidSignReq(*signReq) == CheckResult::VALID);
...
void TestIsValidViewChange(FakeConsensus<FakePBFTEngine>& fake_pbft, ViewChangeReq::Ptr req)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->isValidViewChangeReq(*req, nodeIdxSource) == true);

    /// case 1: own viewchange request
    IDXTYPE orgIdx = fake_pbft.consensus()->nodeIdx();
    fake_pbft.consensus()->setNodeIdx(nodeIdx);
    BOOST_TEST(fake_pbft.consensus()->isValidViewChangeReq(*req, nodeIdx) == false);
    fake_pbft.consensus()->setNodeIdx(orgIdx);

    /// case 2: existed viewchange
    fake_pbft.consensus()->reqCache()->addViewChangeReq(req);
    BOOST_TEST(fake_pbft.consensus()->isValidViewChangeReq(*req, nodeIdxSource) == false);
    fake_pbft.consensus()->reqCache()->clearAll();
    BOOST_TEST(fake_pbft.consensus()->isValidViewChangeReq(*req, nodeIdxSource) == true);

    /// case 6: catchupView(send viewchange message to the source)
    fake_pbft.consensus()->setToView(req->view + 2);
    BOOST_TEST(fake_pbft.consensus()->isValidViewChangeReq(*req, req->idx) == true);
    ...
    CommitReq::Ptr commitReq, PrepareReq::Ptr prepareReq, KeyPair const& peer_keyPair, bool succ)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->isValidCommitReq(*commitReq) == CheckResult::VALID);
...
{
    BOOST_TEST(_first->block_hash == _second->block_hash);
    BOOST_TEST(_first->height == _second->height);
    BOOST_TEST(_first->idx == _second->idx);
    BOOST_TEST(_first->sig == _second->sig);
    BOOST_TEST(_first->timestamp == _second->timestamp);
    BOOST_TEST(_first->view == _second->view);
    BOOST_TEST(_first->sig2 == _second->sig2);
}
...
BOOST_AUTO_TEST_CASE(testInitPBFTEnvNormalCase)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->keyPair().pub() != h512());
    BOOST_TEST(fake_pbft.consensus()->broadCastCache());
    BOOST_TEST(fake_pbft.consensus()->reqCache());
    /// init pbft env
    ...
    BOOST_TEST(fake_pbft.consensus()->consensusBlockNumber() == 0);
    BOOST_TEST(fake_pbft.consensus()->toView() == 0);
    BOOST_TEST(fake_pbft.consensus()->view() == 0);
    /// check resetConfig
    BOOST_TEST(fake_pbft.consensus()->accountType() != NodeAccountType::SealerAccount);
    ...
    fake_pbft.consensus()->resetConfig();
    BOOST_TEST(fake_pbft.consensus()->accountType() == NodeAccountType::SealerAccount);
    BOOST_TEST(fake_pbft.consensus()->nodeIdx() == fake_pbft.m_sealerList.size() - 1);
    BOOST_TEST(fake_pbft.consensus()->sealerList().size() == fake_pbft.m_sealerList.size());
    BOOST_TEST(fake_pbft.consensus()->nodeNum() == fake_pbft.m_sealerList.size());
    ...
    /// check m_timeManager
    BOOST_TEST(fake_pbft.consensus()->timeManager().m_viewTimeout ==
               fake_pbft.consensus()->timeManager().m_emptyBlockGenTime * 3);
    BOOST_TEST(fake_pbft.consensus()->timeManager().m_changeCycle == 0);
    BOOST_TEST(fake_pbft.consensus()->timeManager().m_lastGarbageCollection <=
...
BOOST_AUTO_TEST_CASE(testTimeout)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->toView() == orgToView);
    ...
    fake_pbft.consensus()->checkTimeout();
    BOOST_TEST(fake_pbft.consensus()->toView() == orgToView + 1);
    BOOST_TEST(timeManager.m_changeCycle == orgChangeCycle + 1);
...
BOOST_AUTO_TEST_CASE(testCheckAndChangeView)
{
    ...
    fake_pbft.consensus()->checkTimeout();
    BOOST_TEST(fake_pbft.consensus()->toView() == orgToView + 1);
    BOOST_TEST(timeManager.m_changeCycle == oriChangeCycle + 1);
    ...
    for (size_t i = 1; i < fake_pbft.m_sealerList.size(); i++)
    {
        ...
        if (4 >= size)
        {
            BOOST_TEST(fake_pbft.consensus()->view() == oriview);
        }
        // collect 5 nodes with the same view will trigger view change
        else if (5 == size)
        {
            ///< view change triggered, view += 1
            BOOST_TEST(fake_pbft.consensus()->view() == fake_pbft.consensus()->toView());
            break;
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testCheckAndCommit)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->reqCache()->rawPrepareCache() !=
    ...
    /// BOOST_TEST(fake_pbft.consensus()->timeManager().m_lastSignTime <= utcTime());
}
/// test isValidPrepare
...
BOOST_AUTO_TEST_CASE(testHandlePrepareReq)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->reqCache()->prepareCache().block_hash == req->block_hash);
    /// check broadcastSign
    for (size_t i = 0; i < fake_pbft.m_sealerList.size(); i++)
    {
    ...
    /// BOOST_TEST(fake_pbft.consensus()->reqCache()->prepareCache().block_hash == h256());
    BOOST_TEST(fake_pbft.consensus()->reqCache()->rawPrepareCache() ==
               fake_pbft.consensus()->reqCache()->committedPrepareCache());
    bytes data;
    fake_pbft.consensus()->reqCache()->committedPrepareCache().encode(data);
...
BOOST_AUTO_TEST_CASE(testHandleSignMsg)
{
    ...
    BOOST_TEST(*signReq2 == *signReq);
    /// check the signReq has been added to the cache
    BOOST_TEST(fake_pbft.consensus()->reqCache()->isExistSign(*signReq2));
    CheckBlockChain(fake_pbft, block_number);
    ...
    BOOST_TEST(fake_pbft.consensus()->reqCache()->committedPrepareCache() ==
...
BOOST_AUTO_TEST_CASE(testHandleCommitMsg)
{
    ...
    BOOST_TEST(*commitReq2 == *commitReq);
    BOOST_TEST(fake_pbft.consensus()->reqCache()->isExistCommit(*commitReq2));
    CheckBlockChain(fake_pbft, block_number);
...
BOOST_AUTO_TEST_CASE(testShouldSeal)
{
    ...
    fake_pbft.consensus()->resetConfig();
    BOOST_TEST(fake_pbft.consensus()->shouldSeal() == false);
    ...
    BOOST_TEST(fake_pbft.consensus()->shouldSeal() == true);
    ...
    fake_pbft.consensus()->setNodeIdx(fake_pbft.consensus()->getLeader().second);
    BOOST_TEST(fake_pbft.consensus()->notifyNextLeaderSeal() == false);
    ...
    fake_pbft.consensus()->notifySealing(*block.m_block);
    BOOST_TEST(fake_pbft.consensus()->notifyNextLeaderSeal() == true);
...
BOOST_AUTO_TEST_CASE(testHandleViewchangeMsg)
{
    ...
    BOOST_TEST(*viewchange_req_decoded == *viewchange_req);
    /// viewchange no consensused
    BOOST_TEST(fake_pbft.consensus()->view() != viewchange_req_decoded->view);
...
BOOST_AUTO_TEST_CASE(testFastViewChange)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->toView() != viewchange_req->view - 1);
    ...
    fake_pbft.consensus()->checkTimeout();
    BOOST_TEST(fake_pbft.consensus()->view() == viewchange_req->view);
}
...
BOOST_AUTO_TEST_CASE(testCheckBlock)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->checkBlock(
    ...
    BOOST_TEST(fake_pbft.consensus()->checkBlock(*block.m_block) == false);
    fake_pbft.consensus()->setSealerList(block.m_block->blockHeader().sealerList());
    BOOST_TEST(fake_pbft.consensus()->checkBlock(*block.m_block) == true);
    ...
    BOOST_TEST(fake_pbft.consensus()->checkBlock(*invalid_block.m_block) == false);
}
...
BOOST_AUTO_TEST_CASE(testCreatePBFTReqCache)
{
    ...
    BOOST_TEST(std::dynamic_pointer_cast<PartiallyPBFTReqCache>(
...
BOOST_AUTO_TEST_CASE(testHandlePartiallyPrepare)
{
    ...
    BOOST_TEST(sendedPrepare->idx == prepareReq->idx);
    BOOST_TEST(sendedPrepare->view == prepareReq->view);
    ...
    BOOST_TEST(receivedP2pMsg != nullptr);
    ...
    for (auto tx : *(followerPBFT->consensus()
                         ...
                         ->pBlock->transactions()))
    {
        BOOST_TEST(*tx == *(*sendedPrepare->pBlock->transactions())[i]);
        i++;
    }
}
...
BOOST_AUTO_TEST_CASE(testResetConsensusTimeout)
{
    ...
    BOOST_TEST(timeMgr.isTimeout(now) == true);
    ...
    now += 1000 * 60 * 15;
    BOOST_TEST(timeMgr.isTimeout(now) == false);
    auto invalidNow = now + 1000 * 60 * 15 + 100;
    // execution timeout
    BOOST_TEST(timeMgr.isTimeout(invalidNow) == true);

    // case3: receive rawPrepare, and execution without timeout
    timeMgr.m_lastExecTime = now;
    BOOST_TEST(timeMgr.isTimeout(timeMgr.m_lastExecTime + 2000) == false);
    // network timeout
    ...
    now = invalidNow + 3;
    BOOST_TEST(timeMgr.isTimeout() == true);
    ...
    BOOST_TEST(timeMgr.isTimeout(now + 3001) == true);
...
BOOST_AUTO_TEST_CASE(testViewChange)
{
    ...
    BOOST_TEST(fake_pbft.consensus()->reqCache()->latestViewChangeReqCache()->size() == 4);
    BOOST_TEST(fake_pbft.consensus()->reqCache()->latestViewChangeReqCache()->count(0) == true);
    // erase the session
    ...
    BOOST_TEST(fake_pbft.consensus()->reqCache()->latestViewChangeReqCache()->size() == 3);
    BOOST_TEST(fake_pbft.consensus()->reqCache()->latestViewChangeReqCache()->count(0) == false);
