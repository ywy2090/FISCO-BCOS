...
bcos::Error::UniquePtr decode(bcos::bytesRef& in, AuthorizationListEntry& out) noexcept
{
    ...
    if (auto error = decode(in, out.chainId); error != nullptr)
    {
        return error;
    }
    if (auto error = decode(in, out.address); error != nullptr)
    {
        return error;
    }
    if (auto error = decode(in, out.nonce); error != nullptr)
    {
        return error;
    }
    {
        uint64_t yParity = 0;
        if (auto error = decode(in, yParity); error != nullptr)
        {
            return error;
        }
        out.yParity = static_cast<uint8_t>(yParity);
    }
    if (auto error = decode(in, out.r); error != nullptr)
    {
        return error;
    }
    if (auto error = decode(in, out.s); error != nullptr)
    {
        return error;
    }
    return nullptr;
