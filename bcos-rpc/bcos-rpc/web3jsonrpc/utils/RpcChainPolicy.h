...
/// predicate, distinct from the ledger's feature_l2_ethereum_compat state shape: the Eth lane
/// may carry that flag too (an MPT-state chain still sealed by the consensus layer), and on such
/// a chain the OP-specific fee rule (the fee-history reconstruction's tip floor) must not apply.
