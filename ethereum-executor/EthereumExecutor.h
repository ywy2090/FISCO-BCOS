...
public:
        ...
        int contextID, ledger::LedgerConfig const& ledgerConfig, bool call)
    {
        ...
        try
        {
            ...
            if (!revOpt.has_value())
            {
                BOOST_THROW_EXCEPTION(std::runtime_error("evmcRevision not configured"));
            }
            auto rev = *revOpt;
            ...
            int64_t initcodeCost =
                (isCreate && rev >= EVMC_SHANGHAI) ?
                    INITCODE_WORD_COST * static_cast<int64_t>((input.size() + 31) / 32) :
                    0;

            int64_t intrinsicCost =
                TX_BASE_COST + createCost + dataCost + accessListCost + authListCost + initcodeCost;

            int64_t minCost =
                (rev >= EVMC_PRAGUE) ? TX_BASE_COST + numTokens * TOTAL_COST_FLOOR_PER_TOKEN : 0;
            ...
            {
                auto senderAcc = stateView.get_account(evmTx.sender)
                                     .value_or(evmone::state::StateView::Account{});
                auto const baseFee =
                ...
                if (evmTx.max_gas_price < evmTx.max_priority_gas_price)
                    BOOST_THROW_EXCEPTION(PriorityGasPriceExceedsMax{} << bcos::errinfo_comment(
                ...
                auto maxTotalFee = intx::umul(intx::uint256{evmTx.gas_limit}, evmTx.max_gas_price);
                ...
                if (evmTx.type == evmone::state::Transaction::Type::blob)
                {
                    ...
                    if (evmTx.blob_hashes.empty())
                        BOOST_THROW_EXCEPTION(EmptyBlobVersionedHashes{}
                    ...
                    if (evmTx.max_blob_gas_price < bbf)
                        BOOST_THROW_EXCEPTION(BlobFeeCapBelowBaseFee{} << bcos::errinfo_comment(
            ...
            co_await applyStateDiff(
                storage, evmReceipt.state_diff, rev, *m_hashImpl, storageTracker);
        ...
        std::optional<uint64_t> blockReward)
    {
        ...
        auto diff = evmone::state::finalize(stateView, rev, coinbase, blockReward, {}, {});
