#include "bcos-evm/specs-tests/EestStateTestLoader.h"
...
std::filesystem::path moduleAssetsRoot()
{
    ...
    return std::filesystem::path(SPECS_TESTS_SOURCE_DIR) / "assets" / "eest";
#else
    return std::filesystem::path("bcos-evm/specs-tests/assets/eest");
#endif
}
