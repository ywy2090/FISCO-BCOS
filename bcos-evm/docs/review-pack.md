...
实现：`eth/kernel/state-transition/TxPipeline.cpp`
...
文件：`eth/kernel/state-transition/TxPipelineHooks.h`
...
**纪律：** `eth/kernel/state-transition/` 及 portable headers **不得** `#include bcos/` 或 `opstack/`。链代码通过 hook lambda 在 wrapper 翻译单元注入。
...
| 类别 | 字段 | 语义 |
| --- | --- | --- |
| **A 类 feature-gated** | `eip2929`, `eip2537`, `eip7212`, `eip7623`, `eip7823`, `eip7702` | FISCO 需显式 `Features::Flag` |
...
1. `eth/kernel/state-transition/` 出现 chain include
...
| 共享编排管线 | `eth/kernel/state-transition/TxPipeline.cpp` |
| 编排上下文 / 钩子 | `eth/kernel/state-transition/StateTransitionContext.h`、`TxPipelineHooks.h` |
| intrinsic 扣减 | `eth/kernel/state-transition/DeductIntrinsicGas.h` |
| 内核入口 | `eth/kernel/execution/InnerExecute.h` / `.cpp` → `TxExecutionRunner` |
