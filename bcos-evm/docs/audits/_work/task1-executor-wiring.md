...
| RevisionConfig `warm_access` (Isthmus) | revision profile | matrix feature-gated (profile-only) | feature-gated | smoke | ✅ | ADR-004 profile-only; EthPolicy Berlin+ | `makeIsthmusRevisionConfig` `warm_access=true` (`RevisionConfig.h:66`) | runtime 2929 via revision + flag | `RevisionConfigProfileTest::isthmus_helper_sparse_profile_all_fields` | profile-only 语义仍适用 |
