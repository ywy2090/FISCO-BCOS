...
| # | Question | Choice |
| --- | --- | --- |
| D1 | Seam placement | **A.** New kernel module `eth/kernel/state-transition/ExecutionSession` (not construction-valid `TxPipelineContext`, not `OrchestrationProfile::Session`). |
...
### 2. Kernel module: `eth/kernel/state-transition/ExecutionSession.h`
