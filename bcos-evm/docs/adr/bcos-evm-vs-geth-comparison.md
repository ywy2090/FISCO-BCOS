...
| 帧模型 | `runExecutionFrame(CallFrameContext, msg, FrameScope, host)` — 统一帧执行，scope=TopLevel/Nested | `evm.interpreter.Run` 递归，`evm.depth` 跟踪 | 显式 scope vs 隐式 depth，语义等价 | 低 |
...
| 钩子注入点 | EvmHostHooks(6), StateTransitionHooks(7), StateTransitionErrorPolicy(4) | 无等价物 | **FISCO 独有可扩展性，但钩子可能改变 EVM 语义** | **中** |
...
| 6 | **钩子语义泄漏** | `EvmHostHooks` / `StateTransitionHooks` | 钩子可改变标准 EVM 行为 |
