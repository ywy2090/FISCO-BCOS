...
链条：CREATE 到地址 A → 碰撞检查读 A（`eth/kernel/execution/CreateDeployment.h:79`）⇒ memo 缓存"A 不存在" → `FiscoEvmHostHooks::bumpContractCreateNonce`（`bcos/FiscoEvmHostHooks.cpp:133`）经 `persistContractCreateNonce`（`transaction-executor/.../TransactionExecutorImpl.h:261`）**执行中途**把 A 的 nonce 直写进 `LedgerStateView` 底下的存储 → 内层 revert 撤掉 A 的 overlay 条目 → 同 tx 再读 A：现行代码见 nonce，memo 版见过期"不存在"。另两处 FISCO 写入（`buyGas` 预扣、`refundGas` 回滚）经时序核实位于 `State` 生存期外（`TransactionExecutorImpl.h:166/169/183`），本就安全。**用户裁定（2026-07-09）：「后续版本不支持 FISCO 路径，可以不考虑」——此场景不予考虑，也不为其实现禁用开关。**
