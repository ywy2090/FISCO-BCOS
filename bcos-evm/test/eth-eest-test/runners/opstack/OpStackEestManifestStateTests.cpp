...
///     --manifest opstack-eest-state-smoke.json \
///     --eest-root <dir> \
///     --expectations opstack-expectations.json
...
Options parseOptions(int argc, char** argv)
{
    ...
    options.expectationsPath =
        std::filesystem::path(SPECS_TESTS_MANIFEST_DIR) / "opstack-expectations.json";
#else
    options.expectationsPath = std::filesystem::path("manifests/opstack-expectations.json");
#endif
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        std::cout << "All " << executed << " EEST state subtest(s) passed via OPStack path\n";
        return 0;
    }
    catch (std::exception const& ex)
    {
        std::cerr << "OpStackEestManifestStateTests error: " << ex.what() << '\n';
        return 1;
    }
}
