...
```bash
cmake -S . -B build-ref -DTESTS=ON -DBCOS_EVM_REFERENCE_TESTS=ON \
  -DBCOS_EVM_REFERENCE_TESTS_FETCH_ASSETS=OFF
...
PR CI (`capability-gate`) configures with `BCOS_EVM_REFERENCE_TESTS=ON` (FetchContent downloads assets) and runs smoke via `ctest -L evm-reference-tests-smoke`.
