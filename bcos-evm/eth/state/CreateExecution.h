/*
 * CREATE / CREATE2 execution setup (geth evm.create parity).
 */
#pragma once
...
inline evmc_address predictCreateAddress(
    State& state, evmc_message const& message, bcos::bytesConstRef initCode) noexcept
{
    if (!isZeroAddress(message.recipient))
    {
        return message.recipient;
    }
    if (!isZeroAddress(message.code_address))
    {
    ...
    return predictLegacyCreateAddress(message.sender, state.get_nonce(message.sender));
}

/// Bind CREATE recipient/code_address from sender nonce (no state mutation).
inline void bindCreateMessageForInit(
    EthHost& host, evmc_message& message, bcos::bytesConstRef initCode, State& state) noexcept
{
...
inline void initializeCreateTargetAccount(
    State& state, evmc_address const& createAddr, evmc_revision revision, bool warmAccess) noexcept
{
    if (isZeroAddress(createAddr))
    {
        return;
...
inline void prepareCreateTargetBeforeInit(State& state, EthHost& host, evmc_message& message,
