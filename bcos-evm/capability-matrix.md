...
| FISCO chain precompile dispatch via Port | orchestration | unsupported | explicit (`applyFiscoMessage` injects `ChainPrecompileDispatch` / `AuthPort` and dispatches in TE adapters) | unsupported | `ExecuteViaHostCompat` |
