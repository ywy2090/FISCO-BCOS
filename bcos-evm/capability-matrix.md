...
**Orchestration pipeline (ADR-019):** All three chain L1 adapters converge on sync `stateTransitionExecute` in `eth/pipeline/StateTransitionExecute.*`. Chain-specific precheck, intrinsic policy, fee routing (`buyGas`/`refundGas`), deposit state machine, and final `stateDiff`/`logs` mapping remain in thin wrappers per ADR-005.
...
| FISCO chain precompile dispatch via Port | orchestration | unsupported | explicit (`applyFiscoMessage` injects `ChainCallTargetDispatcher` / `AuthPort` and dispatches in TE adapters) | unsupported | `ExecuteViaHostCompat` |
...
| RevisionConfig `eip1559` | revision profile | inherited (TE consumer via `Eip1559Access.h`; typed-tx gate, fee-cap precheck, gas refund) | inherited (via `revisionConfigFromRevision` at CANCUN floor+, ADR-004/018) | inherited (via `derive(PRAGUE)`; `Eip1559Access.h`) | `RevisionConfigProfile`, `Eip1559Access`, `EthEip1559Gas` |
