...
| RevisionConfig `eip6780` | revision profile | inherited (via `EthPolicy` at CANCUN+; kernel consumer wired in `EthHost::selfdestruct`) | inherited (via `FiscoPolicy` at CANCUN+) | inherited (via Isthmus `revision`) | `RevisionConfigProfileTest` |
...
| RevisionConfig `eip7823` | revision profile | inherited (via `EthPolicy` at OSAKA+; TE consumer wired in `EthPrecompiles` modexp dispatch) | feature-gated (`feature_evm_osaka`; TE consumer wired when flag ON) | feature-gated (not set on Isthmus helper) | `RevisionConfigProfileTest`, `Eip7823ModexpRejectTest`, `Bcos7823ModexpRejectTest` |
