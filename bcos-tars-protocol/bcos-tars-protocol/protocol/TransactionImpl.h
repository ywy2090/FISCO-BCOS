...
private:
    // NOTE (interim, #5366 vendor): this branch's cached web3AccessList()
    // (ensureWeb3AccessListCache
    // + m_web3AccessListCache*) was superseded by #5366's non-cached web3AccessList() impl during
    // the vendor merge. The cache (perf-only; identical results) is restored on the eventual #5366
    // rebase, when this file reverts to the branch's original.
